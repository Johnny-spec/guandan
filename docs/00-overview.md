...
- [x] **Phase 3 · Sprint 3 (Adaptive Card 渲染快照测试)** — `packages/adaptive-cards/src/builders.ts`：TS 类型化的 Adaptive Card 1.5 子集（TextBlock / ColumnSet / FactSet / Container / Action）+ 4 个 builder：`buildWelcomeCard` / `buildRoomCreatedCard` / `buildMatchFinishedCard`（含 rating delta、用时格式化）/ `buildRefereeActionCard`（6 种 kind 各自 emoji + Container style）。新增 vitest devDep + `cards.test.ts` 17 测试 / 9 snapshot；snapshot 锁定 schema / version / verb 名（防 Teams 端解析漂移）。→ 209/209（17 + 55 + 137）。
- [x] **Phase 3 · Sprint 3 (AI 难度调优 — 困难档队友信号)** — `apps/game-server/src/ai/strategy.ts` 重写：新增 `SeatCode` 类型 + `topOwnerSeat?` 字段 + `partnerOf()` 辅助；`rateLead` 引入 `sizeBonusFactor` 参数。hard 模式三项升级：（1）起手 `sizeBonusFactor` 0.25（vs 默认 0.1）鼓励多牌组合，加快手牌消耗；（2）队友持顶 + 队友手牌 ≤8 + 对手更安全 + 自己 ≥6 张 → 让位 pass 不压队友；（3）队友持顶 + 队友 ≤3 张 → 不浪费炸弹帮清桌。`game.session.snapshotForBot` 暴露 `topSeat`；`bot.service` 透传 `topOwnerSeat`。新增 7 测试（4 条队友信号案例 + 1 条 backward-compat + 2 条 size bonus）；旧 7 strategy 测试整套零修改通过（`topOwnerSeat` 可选 → partner-aware 分支不触发）。→ 216/216（17 + 55 + 144）。
- [x] **Phase 3 · Sprint 3 (Spectator Teams Meeting Extension — 占位 + manifest)** — `packages/teams-sdk-wrapper/src/meeting.ts`：`buildSpectatorConfig` (roomId/hostUrl → contentUrl/entityId，roomId 走白名单防路径注入) + `buildMeetingConfigurableTab` (host → 完整 configurableTab 形态：scope groupchat + context meetingSidePanel/Stage/ChatTab) + `setSpectatorConfig`/`getMeetingContext` Teams SDK 包装。`apps/teams-tab/app/meeting/configure/page.tsx`（roomId 表单 + `pages.config.setConfig` + `setValidityState` 联动）+ `app/meeting/spectate/page.tsx`（meeting context landing 页）。`appPackage/manifest.json` 把 `configurableTabs[]` 填入 meeting tab（指向 `/meeting/configure`）。新增 8 测试（含 2 snapshot 锁 contentUrl/entityId/context 形态 + manifest 契约校验解析 manifest.json 断言 configurableTab 形态一致）。→ 224/224（17 + 8 + 55 + 144），teams-tab `next build` 通过。
