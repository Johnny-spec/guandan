...
| Tournament 自动开局调度 | Server | ✅ Done（`apps/game-server/src/tournament/tournament.scheduler.ts`：`TournamentScheduler` 调度器扫描 OPEN 赛事，三类自动决策——①deadline 已过 + 活跃 ≥2 → AUTO_START；②deadline 已过 + 活跃 <2 → AUTO_CANCEL；③活跃数达到 maxTeams → AUTO_START。`tickOnce(now)` 纯方法可注入时间；`Clock` 可注入；`OnModuleInit` 用可关闭的 `setInterval`（默认 30s，`enableInterval` 选项控制）；bounded ring buffer 记录最近动作。新 REST `POST /api/v1/tournaments/scheduler/tick`（运维手动触发） + `GET /api/v1/tournaments/scheduler/recent?limit=`。单个 tournament 失败被捕获为 ERROR action 不影响其他赛事。+12 测试 / 369 green） |
...
| Guild 频道 / 活动（基础） | Server/Frontend | ✅ Done（`apps/game-server/src/guild/guild-activity.{repository,service,controller}.ts`：`GuildChannelRecord`（TEXT/ANNOUNCEMENT × ACTIVE/ARCHIVED，正则 `^[\u4e00-\u9fa5A-Za-z0-9_\- ]{2,32}$`，ACTIVE 内大小写去重，归档名可复用）+ `GuildEventRecord`（SCHEDULED/CANCELLED/COMPLETED，ISO `scheduledAt`，可选正整数 `capacity`，`hostUserId` 默认请求者，外部 host 必须 ACTIVE）+ `GuildEventRsvpRecord`（GOING/INTERESTED/DECLINED，(eventId,userId) 幂等 upsert）；RBAC：channel 创建/归档限 OWNER+ADMIN，event 创建限 OWNER+ADMIN，cancel 允许 host，list/rsvp 限 ACTIVE 成员；`GOING` RSVP 入库前校验 `capacity`（排除自身、降档释放名额）；DISBANDED 公会拒绝新建 channel/event（`GUILD_DISBANDED`）；REST 拆三个控制器规避路由冲突（`/guilds/:guildId/{channels,events}` GET/POST、`/guild-channels/:channelId/archive`、`/guild-events/:eventId/{cancel,rsvp,rsvps}`）；+22 测试 / 391 green） |
